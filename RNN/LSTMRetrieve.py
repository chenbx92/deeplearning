...
with tf.Session() as sess:
    ...
    correct_pred=tf.equal(tf.argmax(predict,1),tf.argmax(input_y,1))
    ...
    '''
    x = mnist.test.images.reshape([-1, 28, 28])
    res = sess.run(predict, feed_dict={input_x: x})
    print("actual class:", str(sess.run(tf.argmax(mnist.test.labels, 1))))
    print("predicted class:", str(sess.run(tf.arg_max(res, 1))))
    print(sess.run(accuracy, feed_dict={input_x: x, input_y: mnist.test.labels}))